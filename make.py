...
def show_help():
    ...
    print()
    print("示例:")
    print("  python make.py build")
    print("  python make.py release 1.2.0 '添加新功能' '修复bug'")
    print("  python make.py patch '修复重要bug'")
    print("  python make.py minor '添加忽略重复文件功能'")
    print("  python make.py major '重构核心架构'")
...
def cmd_build():
    ...
    if os.path.exists('build_app.py'):
        return run_command(['python', 'build_app.py'], "构建应用程序")
    else:
        print("❌ 未找到 build_app.py 构建脚本")
        return False
...
def cmd_release(version, changes):
    ...
    if changes:
        cmd.extend(changes)
    return run_command(cmd, f"发布版本 {version}")
...
def main():
    ...
    if command in ['help', '--help', '-h']:
        ...
        if len(args) < 1:
            print("❌ 请指定版本号")
            print("用法: python make.py release <version> [changes...]")
            sys.exit(1)
        ...
        print("使用 'python make.py help' 查看可用命令")
        sys.exit(1)
