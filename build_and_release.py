...
def show_help():
    ...
    print()
    print("用法:")
    print("  python build_and_release.py <新版本号> [选项] [更新内容...]")
    print()
    print("参数:")
    ...
    print("示例:")
    print("  # 更新版本并完整发布")
    print("  python build_and_release.py 1.2.0 '添加新功能' '修复重要bug'")
    print()
    print("  # 仅构建不发布")
    print("  python build_and_release.py 1.2.0 --build-only '添加新功能'")
    print()
    print("  # 跳过构建直接发布（假设已经构建过）")
    print("  python build_and_release.py 1.2.0 --skip-build '修复bug'")
...
def update_version(new_version, changes):
    ...
    # 构建更新版本的命令
    cmd = ['python', 'update_version.py', new_version]
    if changes:
        cmd.extend(changes)
...
def build_application():
    ...
    # 检查是否存在构建脚本
    build_scripts = ['build_app.py', 'build.py', 'setup.py']
    build_script = None
    ...
    if not build_script:
        print("❌ 未找到构建脚本 (build_app.py, build.py, setup.py)")
        return False
...
def run_release_script():
    ...
    if not os.path.exists('release.py'):
        print("❌ 未找到 release.py 发布脚本")
        return False
    
    result = subprocess.run(['python', 'release.py'])
    if result.returncode != 0:
        print("❌ 发布失败!")
        return False
...
def check_working_directory():
    """检查工作目录"""
    required_files = ['version.py', 'update_version.py']
    missing_files = []
...
def main():
    ...
    try:
        ...
        if build_only:
            print("🔨 构建完成，可以手动运行 python release.py 进行发布")
        else:
            print("🚀 构建和发布完成!")
