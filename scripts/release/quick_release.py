...
def main():
    ...
    # 步骤1: 构建应用
    success, _ = run_command("python scripts/build/build_app.py", "构建应用")
    if not success:
        sys.exit(1)
    ...
    if auto_release:
        print("🚀 启动自动发布...")
        success, _ = run_command("python scripts/release/release.py", "自动发布")
    else:
        print("📋 启动手动发布...")
        success, _ = run_command("python scripts/release/release.py", "准备手动发布")
